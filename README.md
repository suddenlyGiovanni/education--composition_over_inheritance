# education--composition_over_inheritance
following MPJ video on Composition over Inheritance
```typescript
...
const makeKillerRobotDog = (name: string) => {
  ...
  return Object.assign({}, barker(state), pooper(state), killer(state), driver(state))
}
